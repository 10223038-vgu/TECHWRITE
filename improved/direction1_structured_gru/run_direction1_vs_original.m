...
if isfile(cachedNet)
    ...
    N = 256; L = 4; nRealizations = 40; maxEpochs = 40;
    Bc = coherenceBandwidth(N, L);
    Lseq = round(Bc);
    [seqCorr, ~] = buildCorrelatedAndShuffledSequences(M, 8, Lseq, N, L, 6, nRealizations, mlpNet_orig);
    gruNet_d1 = trainGRUFromSequences(seqCorr, 2, 100, maxEpochs);
    mlpNet_d1 = mlpNet_orig;
end

ber_direction1 = simulateBER('deeplas', M, SNRdB_range, nBlocksMin, mlpNet_d1, gruNet_d1);
