function BER = simulateBERPooling(M, SNRdB_range, nBlocksMin, mlpNet, poolNet, Lseq, N, L, tau, minErrors, maxBlocksFactor)
% SIMULATEBERPOOLING BER simulator for the pooling-net Deep LAS detector.
% Same role as simulateBERDirection1.m, but builds the fixed-size
% permutation-invariant pooled feature vector (see
% buildPooledContextFeatures.m) at test time instead of an ordered
% sequence, matching how poolNet (from trainPoolingNet.m) was trained.
...
if N < Lseq
    error('simulateBERPooling:badN', 'N must be >= Lseq.');
end
nSeqPerRealization = floor(N / Lseq);
if nSeqPerRealization < 1
    error('simulateBERPooling:badLseq', 'Lseq must be <= N.');
end
...
for si = 1:numel(SNRdB_range)
    ...
    BER(si) = nErr / nBits;
    fprintf('[pooling-net, %d-QAM] SNR=%2d dB -> BER=%.3e (%d errors / %d bits, %d target subcarriers)\n', ...
